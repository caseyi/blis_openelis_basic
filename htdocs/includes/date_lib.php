...
class DateLib
{
	...
	public static function dobToAge($dob)
	{
		...
		*/
		...
		#editted by Glen to enable display of dates past Unix Epoch timestamp limits
		$today = date('m-d-Y');			//m-d-Y format necessary for gregoriantojd calculation
		...
		$dob_formatted = $dob_array[1]."-".$dob_array[2]."-".$dob_array[0];
		$diff = round(floor(DateLib::dateDiff("-", $today, $dob_formatted)*60*60*24), 0);
		$value = "";
